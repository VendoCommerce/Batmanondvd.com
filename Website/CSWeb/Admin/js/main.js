...
/* add menu active link when on current page - simple checks if class value is same as body class value and adds active to that li element */	
	$(function(){
	$('#primary-nav li a').each(function(){
	if ($(this).attr('class') == $("#pageid").attr('class')) {
...
// make current report link disabled 
$(function(){
	$('.report-nav li a').each(function(){
	if ($(this).parent().attr('class') == $("#pageid").attr('class')) {
    $(this).addClass("disabled");
}
});
});

// name the reports here 
if ($("#pageid").hasClass("standardreport")) {
	$('.reportname').html('<i class="icon-star-empty"></i> Standard Report');
}
if ($("#pageid").hasClass("versionreport")) {
	$('.reportname').html('<i class="icon-list"></i> Version Report');
}
if ($("#pageid").hasClass("sidreport")) {
	$('.reportname').html('<i class="icon-bullhorn"></i> SID Report');
}

// in standard report add green shading and text to revenue total row 
$("#summary table td:contains('Total Revenue')").closest('tr').addClass('success');


// find empty rows in summary table in standard report and give it special class 
	$('#summary table tr td').each(function(index, element) {
	if ($(this).html() == "&nbsp;") {
    $(this).parent().addClass("emptyrow");
}
	else {
...
// if only reports tab is active (client user id) make it active and show reports on main.aspx page 			
	if ($('#dash-example-tabs li').length == 1) {
		$('#reporttab').addClass("active");
		$('#dash-example-tabs-reports').addClass("active");
	}
	else {
    ...
    var uiDemo = function () {
      ...
      /*  var exampleAdvancedDateRange = $('#example-advanced-daterangepicker');
        ...
        function(start, end) {
            exampleAdvancedDateRangeSpan.html(start.toString('MMMM d, yyyy') + ' - ' + end.toString('MMMM d, yyyy'));
        }
        );*/

        // Set the default content when page loads
     /*   exampleAdvancedDateRangeSpan.html(Date.today().toString('MMMM d, yyyy') + ' - ' + Date.today().toString('MMMM d, yyyy'));*/
    };
